...
type WeatherDataSource = "backend" | "direct";
...
function requestedDataSource(): WeatherDataSource {
...
async function fetchJson<T>(url: URL, signal?: AbortSignal): Promise<T> {
  ...
  if (!response.ok) {
    let message = "Weather request failed. Please try again.";
...
async function searchOpenMeteoLocations(
  query: string,
  signal?: AbortSignal,
): Promise<LocationOption[]> {
